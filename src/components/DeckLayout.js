...
function DeckLayout(props) {
  const {
    seatsData: { ticketDetails },
    seatsData,
    updateSeatsData,
  } = useContext(SeatDataContext);
  ...
  const handleSubmit = (e) => {
    try {
      ...
      const emailExists = Object.values(seatsData.ticketDetails).some(
        (ticket) => ticket.email === formData.email
      );
      if (emailExists) {
  ...
  const handleSeatClick = (seatType, seat) => {
    ...
    <>
      <div className="deck-layout bg-white p-1 border-left">
        <div className="flex deck-main">
          <i
            style={{ visibility: props.showSteeringIcon }}
            className="ri-steering-2-fill text-2xl"
          ></i>

          <div
            className={`${
              props.deckType === "lower" ? "border-left-2" : ""
            } h-200 ml-1 p-1 ${
              props.deckType === "lower" ? "deck-lower" : "deck-upper"
            }`}
          >
              ...
              {seatsData[props.deckType]["normal"].map((seat, index) => (
                <div
                  className={`seat text-center ${
                    isOccupied(seat.id) ? "occupied" : "cursor"
                  }`}
                  key={seat.id}
                  onClick={() => handleSeatClick("normal", seat)}
                >
                  {index + 1}-N
                  <div
                    className={`seat-chair ${
                      isOccupied(seat.id) ? "occupied" : ""
                    }`}
                  ></div>
                </div>
              ))}

              {seatsData[props.deckType]["sleeper"]
                .filter((s) => s.type === "back")
                .map((seat, index) => (
                  <div
                    className={`${
                      props.deckType === "upper"
                        ? "spec-seat-upper"
                        : "spec-seat"
                    } ${isOccupied(seat.id) ? "occupied" : "cursor"}`}
                    onClick={
                  ...
                  >
                    <div className="spec-seat-chair"></div>
                  </div>
                ))}
        ...
        <div className=" flex flex-col justify-center items-center">
          <div className="card w-400 p-2">
            ...
            >
              ...
              <button
                type="submit"
                className="p-1 mt-3 bg-primary cursor text-white"
              >
                BOOK
              </button>
            </form>
