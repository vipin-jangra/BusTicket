...
function Dashboard(){
    ...
    const emailExists = Object.values(seatsData.ticketDetails).some(ticket => (ticket.email === formData.email && ticket.seatId !== id));
    if (emailExists) {
        ...
        <div className="dash-main">
            <div className="dash-table">
                <div className="table-div">
                    <BasicTable columns = {Tickets} data = {data ?? []} onView={handleViewTickets} onDelete ={handleDeleteTicket} />   
                </div>

            </div>
        </div>
        <Modal isOpen={modalOpen} onClose={closeModal} modalType = {modalType} onEdit={handleEdit} onUpdate={handleUpdateTicket}>
            <div className=" flex flex-col justify-center items-center">
                <div className='card w-400 p-2'>
                    <form className=" flex flex-col" >

                        <input  type="hidden"  name="deckType"   disabled={disabled} value={deckType}/>
                        ...
                        <input className='p-1 pl-15 mb-1' type="text"  name="firstname" placeholder="Firstname"  disabled={disabled} value={firstname} onChange = {(e)=>{setFirstName(e.target.value)}} />

                        <label className='text-sm text-grey mb-05' htmlFor='email'>Lastname</label>
                        <input className='p-1 pl-15 mb-1' type="text"  name="lastname" placeholder="Lastname" disabled={disabled} value={lastname} onChange={(e)=>{setLastName(e.target.value)}} />
                    
                        <label className='text-sm text-grey mb-05' htmlFor='password'>Email</label>
                        <input className='p-1 pl-15 mb-1' type="email"  name="email" placeholder="Email" disabled={disabled} value={email} onChange={(e)=>{setEmail(e.target.value)}}  />

                        
                        {modalType === "open" ?? <button type="submit" className="p-1 mt-3 bg-primary text-white" >BOOK</button>}
